...
; Mateo Roman - Program 2

; Took some help from Claude 3 for breaking down larger functions into smaller functions

; Program will prompt for a character selection (N, D, P, R, G, S) according to criteria, loops until user decides to search
; If the search is empty, quit program, otherwise filter by the search terms.
; Filtered data is output to the command window
...
(define (display-sorted-by-sales games region) ; Already sorted by game sales, so no just maintain order
  (for ([entry games])
...
(define (display-filtered games [region '()])
  (display "Would you like to order by sales or by rating?:\nS) - Sales\nR) - Rating\n")
  (let ([sort-by (display-and-read "")])
    (cond
     ...
     [else (displayln "Invalid entry") (display-filtered games)])))
...
(define (filter-from-criteria search-list games) ; Need to search the games list by each parameter. The first says by what, the second is value, pass rest until empty.
...
; Read CSV file and start the program
